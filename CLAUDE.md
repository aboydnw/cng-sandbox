...
- **Stories surface the active render mode**: `buildLayersForChapter` returns `{layers, renderMetadata}` where `renderMetadata` carries `{renderMode, reason, sizeBytes}`. `StoryRenderer` and `StoryEditorPage` render `RenderModeIndicator` (`src/components/RenderModeIndicator.tsx`) — an info button in the top-right of the map that reveals the active mode and why — so story authors and readers can see whether a chapter is rendering client-side or falling back to server tiles. The indicator carries `data-snapshot-overlay` so it's included in map snapshots.
