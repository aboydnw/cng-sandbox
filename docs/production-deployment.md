...
- The viewer-subdomain CSP additionally allows `https://fonts.googleapis.com` (style-src) and `https://fonts.gstatic.com` (font-src) so themed embeds (`?bodyFont=`/`?headingFont=` params on `/story/:id/embed`) can load Google Fonts. The main-site CSP does not include these origins — the theming path only runs in embed mode
