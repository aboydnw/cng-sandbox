...
- **`/viewer/*` is auth-gated on the main host, public on the viewer subdomain**: `frontend/public/viewer/` contains the pre-built static bundle (viewer HTML + sidecar chunks + `manifest.json`) used by the "Download static bundle (.zip)" feature. On the main host Caddy does not whitelist `/viewer/*`, so downloading the bundle requires the user to be authenticated — this is intentional, since bundle downloads are an author-facing action. On the viewer subdomain (`viewer.<domain>`) all paths are public and `frame-ancestors *` is set in the CSP, so the viewer subdomain can also serve the bundle files to unauthenticated visitors if needed.
...
The `viewer.<domain>` subdomain serves the same SPA bundle but with no basic auth and a CSP that allows `frame-ancestors '*'`. DNS for `viewer.<domain>` must point at the same Hetzner host as the main site; Caddy auto-provisions the cert from the new server block.
...
- `VITE_VIEWER_ORIGIN=https://viewer.<your-domain>` — read at frontend build time and baked into the bundle. The embed snippet generated by the publish dialog uses this as the iframe `src` origin; without it, the snippet falls back to `window.location.origin` (the auth-gated main domain) and end users pasting the snippet will see a login prompt instead of the story.
