...
- **Client-side COG rendering splits into paletted vs continuous paths**: When the user selects `renderMode: "client"` for a raster dataset, `useLayerBuilder` calls `classifyCogRenderPath` (`src/lib/layers/cogDtype.ts`) on the dataset's `dtype` + `isCategorical` flag. `uint8`/`int8` or any integer dtype flagged categorical → `buildCogLayerPaletted` (uses the library default pipeline, honours the file's color table). Everything else → `buildCogLayerContinuous` (float32 → uint8 normalization + viridis colorize shader, caches raw tile data for the pixel inspector). The two paths have different browser size caps enforced by `useMapControls`: paletted tolerates up to 2 GB, continuous caps at 500 MB. Changing the classifier rules means updating both the builder dispatch and the size-cap logic.
