...
describe("createStoryOnServer", () => {
  it("posts to /api/stories and returns the created story", async () => {
    ...
    const response = { id: "s-1", ...story, published: false, created_at: "2026-01-01", updated_at: "2026-01-01" };
    mockFetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(response) });

    const result = await createStoryOnServer(story as any);
    expect(mockFetch).toHaveBeenCalledWith("/api/stories", expect.objectContaining({ method: "POST" }));
...
describe("saveStoryToServer", () => {
  it("patches /api/stories/:id", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith("/api/stories/s-1", expect.objectContaining({ method: "PATCH" }));
  });
});
...
describe("deleteStoryFromServer", () => {
  it("deletes /api/stories/:id", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith("/api/stories/s-1", expect.objectContaining({ method: "DELETE" }));
  });
});
