...
describe("fetchAndInlineAsBase64", () => {
  ...
  it("throws on fetch failure", async () => {
    ...
    await expect(
      fetchAndInlineAsBase64("https://example.com/missing.png")
    ).rejects.toThrow(/404/);
  });

  it("does not include the asset URL in the thrown error", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({ ok: false, status: 403 })
    );
    await expect(
      fetchAndInlineAsBase64("https://example.com/secret.png?token=abc")
    ).rejects.toThrow(/^Failed to fetch asset \(403\)$/);
  });
});
