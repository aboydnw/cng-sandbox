...
function epsgCode(zone: number, hemisphere: "N" | "S"): number {
  return (hemisphere === "N" ? EPSG_UTM_NORTH_BASE : EPSG_UTM_SOUTH_BASE) + zone;
}
...
function utmZoneWkt(zone: number, hemisphere: "N" | "S"): string {
  ...
  const convId = (hemisphere === "N" ? CONVERSION_UTM_NORTH_BASE : CONVERSION_UTM_SOUTH_BASE) + zone;
...
export function generateUtmZones(): Map<number, ProjectionDefinition> {
  ...
  for (let zone = 1; zone <= 60; zone++) {
    for (const hemisphere of ["N", "S"] as const) {
      map.set(epsgCode(zone, hemisphere), parseWkt(utmZoneWkt(zone, hemisphere)));
    }
  }
  return map;
}
