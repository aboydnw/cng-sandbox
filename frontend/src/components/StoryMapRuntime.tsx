...
import { buildLayersForChapter, beatTime } from "../lib/story/rendering";
...
import type { ScrollytellingChapter, OverlayConfig } from "../lib/story";
...
export interface StoryMapRuntimeProps {
...
export function StoryMapRuntime({
  block,
  datasetMap,
  connectionMap,
  onChapterClick,
  registerBridge,
}: StoryMapRuntimeProps) {
  if (block.type === "map") {
    const dataset =
      datasetMap.get(block.chapter.layer_config.dataset_id) ?? null;
    const connection = block.chapter.layer_config.connection_id
    ...
    return (
      <Box
        onClick={
          onChapterClick ? () => onChapterClick(block.chapter.id) : undefined
        }
        cursor={onChapterClick ? "pointer" : undefined}
      >
        <MapChapter
  ...
  if (block.type === "flyover") {
    if (block.chapter.keyframes.length < 2) {
      ...
      const dataset = datasetMap.get(fallback.layer_config.dataset_id) ?? null;
      const connection = fallback.layer_config.connection_id
...
export default StoryMapRuntime;
